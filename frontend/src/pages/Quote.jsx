...
import { CheckCircle, ChevronRight, ChevronLeft, Upload, Loader, Printer } from 'lucide-react';
...
const steps = ['Product Selection', 'Registration', 'Confirmation'];
...
export default function Quote() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.post(`${API}/api/quote`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
      setSubmitted(true);
    ...
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center p-4">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center max-w-sm w-full bg-white p-12 rounded-[40px] border border-border shadow-2xl">
          ...
          <h2 className="text-3xl font-black text-secondary mb-4">Request Sent!</h2>
          <p className="text-muted font-medium mb-10 leading-relaxed">Our experts will get back to you within 24 hours. Check your inbox for confirmation.</p>
          <button onClick={() => window.location.href = '/'} className="btn-primary w-full py-4 text-lg">Back to Home</button>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-bg pt-32 pb-20">
      <div className="container-custom max-w-3xl">
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-6xl font-black mb-10">Get <span className="gradient-text">Custom Quote</span>.</h1>
        ...
        <div className="bg-white rounded-[40px] border border-border shadow-2xl overflow-hidden p-8 md:p-12">
          <AnimatePresence mode="wait">
            {step === 0 && (
              <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <h3 className="text-2xl font-black mb-8">What do you need printed?</h3>
            ...
            {step === 1 && (
              <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <h3 className="text-2xl font-black mb-8">About you.</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                   <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest text-muted mb-2 ml-1">Full Name</label>
                      <input {...form2.register('name')} className="form-input" />
                   </div>
                   <div>
                      <label className="block text-[10px] font-black uppercase tracking-widest text-muted mb-2 ml-1">Phone</label>
                      <input {...form2.register('phone')} className="form-input" />
                   </div>
                   <div className="md:col-span-2">
                       <label className="block text-[10px] font-black uppercase tracking-widest text-muted mb-2 ml-1">Email</label>
                       <input {...form2.register('email')} className="form-input" />
                   </div>
                   <div className="md:col-span-2">
                       <label className="block text-[10px] font-black uppercase tracking-widest text-muted mb-2 ml-1">Design Requirements</label>
            ...
            {step === 2 && (
              <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                <h3 className="text-2xl font-black">Final check.</h3>
                <div className="bg-surface rounded-3xl p-8 space-y-4">
                   ...
                   <div className="flex justify-between items-center pb-4 border-b border-border">
                      <span className="text-xs font-black text-muted uppercase tracking-widest">Selected</span>
                      <span className="font-bold text-secondary">{formData.selectedProducts.length} Items</span>
                   </div>
                   <div className="pt-2">
                       <span className="text-xs font-black text-muted uppercase tracking-widest block mb-4">Selected Products</span>
                       <div className="flex flex-wrap gap-2">
                          {formData.selectedProducts.map(id => (
                            <span key={id} className="px-4 py-2 bg-white border border-border rounded-xl text-xs font-black text-secondary">
                               {products.find(p => p.id === id)?.name} (x{formData.quantities[id] || '?'})
                            </span>
                          ))}
                       </div>
                   </div>
          ...
          <div className="mt-12 flex justify-between gap-4">
             ...
             <button onClick={step < 2 ? handleNext : handleSubmit} disabled={submitting} className="btn-primary flex-1 md:flex-none">
                {submitting ? <Loader className="animate-spin" /> : <>{step < 2 ? 'Continue' : 'Submit Request'} <ChevronRight size={20} /></>}
             </button>
          </div>
        </div>
      </div>
